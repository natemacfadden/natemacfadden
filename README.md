...
- **[dualGNN](https://github.com/natemacfadden/dualGNN)** — Autoregressive GNN for combinatorial sampling (triangulations) under local (validity, fineness) and global (regularity) constraints out of a large population. We use signed-circuit edge features from oriented matroid theory which we show are provably necessary-and-sufficient for these constraints; these features are also symmetry invariant. Pointer-Network-style decoding, trained with cross-entropy then REINFORCE-tuned for uniformity. Has ~92k parameters (~1000x smaller than CYTransformer) and trains in ~7.5h on a single RTX 5060 Ti. Lowest sample bias of any method we tested (tied, at the largest instance, with [Algorithm 1 of arXiv:2008.01730](https://arxiv.org/abs/2008.01730)) and has zero-shot transfer across unseen problem instances. Applied to Calabi-Yau enumeration up to $h^{1,1}=128$.
<!--
