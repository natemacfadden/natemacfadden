...
String-theory Ph.D. candidate at Cornell (Liam McAllister group). I build ML and high-performance algorithms for hard problems in theoretical physics - the geometry and combinatorics of string compactifications (regular triangulations ❤️). Most recent work: [dualGNN](https://github.com/natemacfadden/dualGNN), a symmetry-aware GNN ~1300x smaller than the prior learned baseline ([CYTransformer](https://arxiv.org/abs/2507.03732), ~120M params) on a combinatorial sampling task, while also training faster, generalizing to unseen instances (the baseline doesn't), and scaling to higher $h^{1,1}$. Lead maintainer of [CYTools](https://github.com/LiamMcAllisterGroup/cytools), the standard toolkit for studying Calabi-Yau compactifications in string theory.
